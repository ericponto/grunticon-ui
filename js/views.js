	...
	Views.ResultsView = Backbone.View.extend( /** @lends Views.ResultsView */ {
		...
		render: function() {
			...
			$.when(view.collection.isRead).done(function(){ 
				...
				view.$el.find("#svg-results").html(svgHtml);
				view.$el.find("#png-results").html(pngHtml);
				view.$el.find("#fallback-results").html(fallbackHtml);
				view.$el.find("#svg-icons-results").html(iconHtml);
				view.$el.fadeIn();
